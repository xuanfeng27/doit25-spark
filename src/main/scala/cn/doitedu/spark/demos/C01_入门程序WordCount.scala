...
object C01_入门程序WordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    conf.setMaster("local")  // 以单线程运行
    // conf.setMaster("local[*]")  // 以机器上的cpu线程数来作为并发数
    // conf.setMaster("local[2]")  // 以指定的数字的线程数作为并发数
    ...
    // 1.加载数据（读数据）
    val rdd1: RDD[String] = sc.textFile("data/wordcount.txt")   // a a a a a b c
